...
public class XmlDocument {
    ...
    public XmlDocument(String xmlString) throws Exception {
        ...
        for(int i = 0; i < bodyChildren.size(); i++) {
            ...
            if (childOfBody instanceof NodeSdt && ((NodeSdt) childOfBody).getBindingPropertyName().startsWith("DBindLoopBegin_")) {
            ...
            if (childrenOfVirtualBlock != null) {
                ...
                if (childOfBody instanceof NodeSdt && ((NodeSdt) childOfBody).getBindingPropertyName().equals("DBindLoopEnd")) {
            ...
            case "w:tbl":
                {
                    ...
                    int _dBind_TableList_index = sourceDocument.indexOfBetween("<w:tag w:val=\"DBindTable_", _sdt_index, _tbl_closing_index);
                    if (_tbl_closing_index > 0 &&_sdt_index >= 0 && _dBind_TableList_index >= 0 && _dBind_TableList_index > _sdt_index)
                        newNode = new NodeTbl();
                    else
      ...
      //  theNodeName =  resolveNodeNameFromXmlString(theNodeTemplate);
        ...
        if (matcher.find()) {
            ...
            if (matcher.find()) {
                ...
                if (fullTagName.startsWith("DBindTable_")){
                    bindingProperty = fullTagName.substring("DBindTable_".length());
                    nodeIsFullyInitialized = true;
                }
            }
        }
...
public class NodeVirtualBlock extends NodeDefault  {
    ...
    private void initializeTheNodeState() throws Exception {
        ...
        // bindingProperty
        bindingProperty = ((NodeSdt)children.get(0)).getBindingPropertyName().substring("DBindLoopBegin_".length());
        nodeIsFullyInitialized = true;
